  ...
  return (
    <>
      ...
      <Table rowKey="id" dataSource={casesQuery.data ?? []} columns={[
        { title: "Client", render: (_, record: CaseItem) => record.client?.name ?? "-" },
      ...
      <Modal title="Create Case" open={open} onCancel={() => setOpen(false)} onOk={onCreate}>
        <Form form={form} layout="vertical" initialValues={{ status: "draft" }}>
          <Form.Item name="clientId" label="Client" rules={[{ required: true }]}><Select options={(clientsQuery.data ?? []).map((c) => ({ value: c.id, label: c.name }))} showSearch optionFilterProp="label" /></Form.Item>
